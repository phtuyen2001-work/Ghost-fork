...
COPY ghost/core/ghost-5.101.4.tgz /ghost-build/
...
RUN set -eux; \
	...
	apkDel=; \
	\
	installCmd='gosu node ghost install --archive /ghost-build/ghost-5.101.4.tgz --db mysql --dbhost mysql --no-prompt --no-stack --no-setup --dir "$GHOST_INSTALL"'; \
